...
# Import digitized KM data (os_stk11wt endpoint)

#dual ici + ct
poseidon_os_stk11wt <- import("ipd/dual ici plus chemo/os/stk11 wt/poseidon os stk11wt.csv") |>
  mutate(study = "poseidon")

cm9la_os_stk11wt <- import("ipd/dual ici plus chemo/os/stk11 wt/cm 9LA 6y os stk11wt.csv") |> 
  mutate(study = "cm9la")

#dual ici alone
mystic_os_stk11wt <- import("ipd/dual ici/os/stk11 wt/mystic os stk11wt.csv") |> 
  mutate(study = "mystic")

# Combine individual patient data (IPD)
combined_os_stk11wt <- rbind(poseidon_os_stk11wt,
                              cm9la_os_stk11wt,
                              mystic_os_stk11wt) |> 
...
# ---- Basic sanity checks ----------------------------------------------------
glimpse(combined_os_stk11wt)
stopifnot(all(c("time", "status", "treat", "study") %in% names(combined_os_stk11wt)))

# Make sure treat and study are factors and create numeric treat for some models
combined_os_stk11wt <- combined_os_stk11wt |> 
  mutate(
...
# 1.a Pooled Cox
coxph_pooled <- coxph(Surv(time, status) ~ treat, data = combined_os_stk11wt)
...
print(zph_pooled)
png("output/schoenfeld residuals plots/os_stk11wt.png", width = 6000, height = 4000, res = 300)  
ggcoxzph(
  zph_pooled
)
dev.off() 
...
tiff("output/km plots/os_stk11wt.tiff", width = 6000, height = 4000, res = 300)  
custom_km_plot(combined_os_stk11wt)
dev.off()  


# Reverse Kaplan–Meier: invert status
compute_followup_stats(combined_os_stk11wt)
