...
# Import digitized KM data (os_keap1wt endpoint)

#dual ici + ct
poseidon_os_keap1wt <- import("ipd/dual ici plus chemo/os/keap1 wt/poseidon os keap1wt.csv") |>
  mutate(study = "poseidon")


#dual ici alone
mystic_os_keap1wt <- import("ipd/dual ici/os/keap1 wt/mystic os keap1wt.csv") |> 
  mutate(study = "mystic")


# Combine individual patient data (IPD)
combined_os_keap1wt <- rbind(poseidon_os_keap1wt,
                         mystic_os_keap1wt) |> 
  mutate(treat_num = case_when(treat == "Dual ICI" ~ as.numeric(0),
                               treat == "Dual ICI + CT" ~ as.numeric(1)))

# ---- Basic sanity checks ----------------------------------------------------
glimpse(combined_os_keap1wt)
stopifnot(all(c("time", "status", "treat", "study") %in% names(combined_os_keap1wt)))

# Make sure treat and study are factors and create numeric treat for some models
combined_os_keap1wt <- combined_os_keap1wt |> 
  mutate(
...
# Why: compare modeling choices and decide which inference is appropriate.
# - cluster(study): common baseline hazard but robust SEs (not preferred for multi-study IPD)
# - strata(study): allows a different baseline hazard per study (recommended)
# - coxme: random-effects / frailty model to quantify heterogeneity

# 1.a Pooled Cox
coxph_pooled <- coxph(Surv(time, status) ~ treat, data = combined_os_keap1wt)
...
print(zph_pooled)
png("output/schoenfeld residuals plots/os_keap1wt.png", width = 6000, height = 4000, res = 300)  
ggcoxzph(
  zph_pooled
)
dev.off()
...
tiff("output/km plots/os_keap1wt.tiff", width = 6000, height = 4000, res = 300)  
custom_km_plot(combined_os_keap1wt)
dev.off() 


# Reverse Kaplan–Meier: invert status
compute_followup_stats(combined_os_keap1wt)
...
#     * piecewise HRs (clinically meaningful intervals) or landmark analyses
...
# - Choose knot locations for piecewise analysis based on clinical judgement / KM shape.
