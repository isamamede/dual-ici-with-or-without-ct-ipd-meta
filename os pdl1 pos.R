...
# Import digitized KM data (os_pdl1pos endpoint)

#dual ici + ct
poseidon_os_pdl1pos <- import("ipd/dual ici plus chemo/os/pdl1 pos/poseidon os pdl1pos.csv") |>
  mutate(study = "poseidon")

cm9la_os_pdl1pos <- import("ipd/dual ici plus chemo/os/pdl1 pos/cm 9LA 6y os pdl1pos.csv") |> 
  mutate(study = "cm9la")

#dual ici alone
mystic_os_pdl1pos <- import("ipd/dual ici/os/pdl1 pos/mystic os pdl1pos.csv") |> 
  mutate(study = "mystic")
...
combined_os_pdl1pos <- rbind(poseidon_os_pdl1pos,
                         cm9la_os_pdl1pos,
                         mystic_os_pdl1pos,
                         cm227_os_pdl1pos) |> 
...
# ---- Basic sanity checks ----------------------------------------------------
glimpse(combined_os_pdl1pos)
stopifnot(all(c("time", "status", "treat", "study") %in% names(combined_os_pdl1pos)))

# Make sure treat and study are factors and create numeric treat for some models
combined_os_pdl1pos <- combined_os_pdl1pos |> 
  mutate(
...
# 1.a Pooled Cox
coxph_pooled <- coxph(Surv(time, status) ~ treat, data = combined_os_pdl1pos)
...
print(zph_pooled)
png("output/schoenfeld residuals plots/os_pdl1pos.png", width = 6000, height = 4000, res = 300)  
ggcoxzph(
  zph_pooled
)
dev.off() 
...
tiff("output/km plots/os_pdl1pos.tiff", width = 6000, height = 4000, res = 300)  
custom_km_plot(combined_os_pdl1pos)
dev.off()  


# Reverse Kaplan–Meier: invert status
compute_followup_stats(combined_os_pdl1pos)
...
png("output/tvhr plots/os_pdl1pos.png", width = 6000, height = 4000, res = 300) 
tvhr_plot(combined_os_pdl1pos)
dev.off()
...
png("output/rmst plots/os_pdl1pos.png", width = 6000, height = 4000, res = 300) 
rmst_plot(data = combined_os_pdl1pos, arm0_label =arm0_label, arm1_label = arm1_label, tau = tau) |> 
  print()
dev.off()
