...
# Import digitized KM data (os_stk11mut endpoint)

#dual ici + ct
poseidon_os_stk11mut <- import("ipd/dual ici plus chemo/os/stk11 mut/poseidon os stk11mut.csv") |>
  mutate(study = "poseidon")

cm9la_os_stk11mut <- import("ipd/dual ici plus chemo/os/stk11 mut/cm 9LA 6y os stk11mut.csv") |> 
  mutate(study = "cm9la")

#dual ici alone
mystic_os_stk11mut <- import("ipd/dual ici/os/stk11 mut/mystic os stk11mut.csv") |> 
  mutate(study = "mystic")

# Combine individual patient data (IPD)
combined_os_stk11mut <- rbind(poseidon_os_stk11mut,
                         cm9la_os_stk11mut,
                         mystic_os_stk11mut) |> 
...
# ---- Basic sanity checks ----------------------------------------------------
glimpse(combined_os_stk11mut)
stopifnot(all(c("time", "status", "treat", "study") %in% names(combined_os_stk11mut)))

# Make sure treat and study are factors and create numeric treat for some models
combined_os_stk11mut <- combined_os_stk11mut |> 
  mutate(
...
# 1.a Pooled Cox
coxph_pooled <- coxph(Surv(time, status) ~ treat, data = combined_os_stk11mut)
...
print(zph_pooled)
png("output/schoenfeld residuals plots/os_stk11mut.png", width = 6000, height = 4000, res = 300)  
ggcoxzph(
  zph_pooled
)
dev.off() 
...
tiff("output/km plots/os_stk11mut.tiff", width = 6000, height = 4000, res = 300)  
custom_km_plot(combined_os_stk11mut)
dev.off()  


# Reverse Kaplan–Meier: invert status
compute_followup_stats(combined_os_stk11mut)
...
png("output/tvhr plots/os_stk11mut.png", width = 6000, height = 4000, res = 300) 
tvhr_plot(combined_os_stk11mut)
dev.off()
...
png("output/rmst plots/os_stk11mut.png", width = 6000, height = 4000, res = 300) 
rmst_plot(data = combined_os_stk11mut, arm0_label =arm0_label, arm1_label = arm1_label, tau = tau,
          ypos = 1, xposfactor = 0.5) |> 
  print()
dev.off()
