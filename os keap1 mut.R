...
# Import digitized KM data (os_keap1mut endpoint)

#dual ici + ct
poseidon_os_keap1mut <- import("ipd/dual ici plus chemo/os/keap1 mut/poseidon os keap1mut.csv") |>
  mutate(study = "poseidon")


#dual ici alone
mystic_os_keap1mut <- import("ipd/dual ici/os/all/mystic os keap1mut.csv") |> 
  mutate(study = "mystic")

# Combine individual patient data (IPD)
combined_os_keap1mut <- rbind(poseidon_os_keap1mut,
                         mystic_os_keap1mut) |> 
  mutate(treat_num = case_when(treat == "Dual ICI" ~ as.numeric(0),
                               treat == "Dual ICI + CT" ~ as.numeric(1)))

# ---- Basic sanity checks ----------------------------------------------------
glimpse(combined_os_keap1mut)
stopifnot(all(c("time", "status", "treat", "study") %in% names(combined_os_keap1mut)))

# Make sure treat and study are factors and create numeric treat for some models
combined_os_keap1mut <- combined_os_keap1mut |> 
  mutate(
...
# 1.a Pooled Cox
coxph_pooled <- coxph(Surv(time, status) ~ treat, data = combined_os_keap1mut)
...
print(zph_pooled)
png("output/schoenfeld residuals plots/os_keap1mut.png", width = 6000, height = 4000, res = 300)  
ggcoxzph(
  zph_pooled
)
dev.off()
...
tiff("output/km plots/os_keap1mut.tiff", width = 6000, height = 4000, res = 300)  
custom_km_plot(combined_os_keap1mut)
dev.off() 


# Reverse Kaplan–Meier: invert status
compute_followup_stats(combined_os_keap1mut)
