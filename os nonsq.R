...
# Import digitized KM data (os_nonsq endpoint)

#dual ici + ct
poseidon_os_nonsq <- import("ipd/dual ici plus chemo/os/nonsq/poseidon os nonsq.csv") |>
  mutate(study = "poseidon")

cm9la_os_nonsq <- import("ipd/dual ici plus chemo/os/nonsq/cm 9LA 6y os nonsq.csv") |> 
  mutate(study = "cm9la")

#dual ici alone

cm227_os_nonsq_pdl1pos <- import("ipd/dual ici/os/nonsq/cm227 os nonsq pos.csv") |> 
  mutate(study = "cm227")

cm227_os_nonsq_pdl1neg <- import("ipd/dual ici/os/nonsq/cm227 os nonsq neg.csv") |> 
  mutate(study = "cm227")

# Combine individual patient data (IPD)
combined_os_nonsq <- rbind(poseidon_os_nonsq,
                        cm9la_os_nonsq,
                        cm227_os_nonsq_pdl1pos,
                        cm227_os_nonsq_pdl1neg) |> 
  mutate(treat_num = case_when(treat == "Dual ICI" ~ as.numeric(0),
                               treat == "Dual ICI + CT" ~ as.numeric(1)))

# ---- Basic sanity checks ----------------------------------------------------
glimpse(combined_os_nonsq)
stopifnot(all(c("time", "status", "treat", "study") %in% names(combined_os_nonsq)))

# Make sure treat and study are factors and create numeric treat for some models
combined_os_nonsq <- combined_os_nonsq |> 
  mutate(
...
# 1.a Pooled Cox
coxph_pooled <- coxph(Surv(time, status) ~ treat, data = combined_os_nonsq)
...
print(zph_pooled)
png("output/schoenfeld residuals plots/os_nonsq.png", width = 6000, height = 4000, res = 300)  
ggcoxzph(
  zph_pooled
)
dev.off() 
...
tiff("output/km plots/os_nonsq.tiff", width = 6000, height = 4000, res = 300)  
custom_km_plot(combined_os_nonsq)
dev.off()  


# Reverse Kaplan–Meier: invert status
compute_followup_stats(combined_os_nonsq)
...
png("output/tvhr plots/os_nonsq.png", width = 6000, height = 4000, res = 300) 
tvhr_plot(combined_os_nonsq)
dev.off()
...
png("output/rmst plots/os_nonsq.png", width = 6000, height = 4000, res = 300) 
rmst_plot(data = combined_os_nonsq, arm0_label =arm0_label, arm1_label = arm1_label, tau = tau) |> 
  print()
dev.off()
