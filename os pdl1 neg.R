...
# Import digitized KM data (os_pdl1neg endpoint)

#dual ici + ct
poseidon_os_pdl1neg <- import("ipd/dual ici plus chemo/os/pdl1 neg/poseidon os pdl1neg.csv") |>
  mutate(study = "poseidon")

cm9la_os_pdl1neg <- import("ipd/dual ici plus chemo/os/pdl1 neg/cm 9LA 6y os pdl1neg.csv") |> 
  mutate(study = "cm9la")
...
combined_os_pdl1neg <- rbind(poseidon_os_pdl1neg,
                             cm9la_os_pdl1neg,
                             cm227_os_pdl1neg) |> 
...
# ---- Basic sanity checks ----------------------------------------------------
glimpse(combined_os_pdl1neg)
stopifnot(all(c("time", "status", "treat", "study") %in% names(combined_os_pdl1neg)))

# Make sure treat and study are factors and create numeric treat for some models
combined_os_pdl1neg <- combined_os_pdl1neg |> 
  mutate(
...
# 1.a Pooled Cox
coxph_pooled <- coxph(Surv(time, status) ~ treat, data = combined_os_pdl1neg)
summary(coxph_pooled)

# 2.a cox.zph on pooled model (approximate)
zph_pooled <- cox.zph(coxph_pooled)
print(zph_pooled)
png("output/schoenfeld residuals plots/os_pdl1neg.png", width = 6000, height = 4000, res = 300)  
ggcoxzph(
  zph_pooled
)
dev.off() 
...
tiff("output/km plots/os_pdl1neg.tiff", width = 6000, height = 4000, res = 300)  
custom_km_plot(combined_os_pdl1neg)
dev.off()  


# Reverse Kaplan–Meier: invert status
compute_followup_stats(combined_os_pdl1neg)
